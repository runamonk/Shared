...
namespace zuulWindowTracker
{
    internal class WindowTracker
    {
        ...
        public WindowTracker()
        {
            ...
            _hhook = SetWinEventHook(EventSystemForeground, EventSystemForeground, IntPtr.Zero, _procDelegate, 0, 0, WineventOutofcontext);
        }
        ...
        private void WinEventProc(IntPtr hWinEventHook, uint eventType, IntPtr hwnd, int idObject, int idChild, uint dwEventThread, uint dwmsEventTime) { WindowChanged?.Invoke(hwnd); }

        ~WindowTracker() { UnhookWinEvent(_hhook); }
